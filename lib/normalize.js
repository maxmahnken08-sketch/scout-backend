...
export async function planTrip(prompt, opts = {}) {
  ...
  emit('intent', 'done', { destination: intent.destination, nights: intent.nights || 6 });
  ...
  const [flights, stays, activities, ratings, restaurants, deal] = await Promise.all([
    track('flights', searchKind('flights', intent), (v) => ({ count: v.length })),
    track('stays', searchKind('stays', intent), (v) => ({ count: v.length })),
    track('activities', searchKind('activities', intent), (v) => ({ count: v.length })),
    searchKind('ratings', intent).catch(() => []),
    track('eats', searchKind('restaurants', intent), (v) => ({ count: v.length })),
  ...
  emit('days', 'done', { count: days.length });
