...
export async function planTrip(prompt, opts = {}) {
  ...
  // Only name what actually came back. The old copy promised stays and
  // experiences unconditionally, which read as a lie whenever a provider was
  // unkeyed and returned nothing.
  const found = [
    flights.length ? 'flights' : null,
    stays.length ? 'stays' : null,
    activities.length ? 'experiences' : null,
    restaurants.length ? 'places to eat' : null,
  ].filter(Boolean);
  const foundText = found.length > 1
    ? `${found.slice(0, -1).join(', ')} and ${found[found.length - 1]}`
    : (found[0] || 'the outline');
  const intro =
    `Here's a ${nights}-night ${intent.destination}${ratingNote} trip — ` +
    `${foundText}, ${underOver} your $${budgetLimit} budget.`;
