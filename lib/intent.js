...
const CITY_DB = {
  ...
  nassau: { destination: 'Nassau', country: 'Bahamas', destinationCode: 'NAS' },
  bahamas: { destination: 'Nassau', country: 'Bahamas', destinationCode: 'NAS' },
  jamaica: { destination: 'Montego Bay', country: 'Jamaica', destinationCode: 'MBJ' },
  'montego bay': { destination: 'Montego Bay', country: 'Jamaica', destinationCode: 'MBJ' },
  'punta cana': { destination: 'Punta Cana', country: 'Dominican Republic', destinationCode: 'PUJ' },
  aruba: { destination: 'Aruba', country: 'Aruba', destinationCode: 'AUA' },
  bermuda: { destination: 'Bermuda', country: 'Bermuda', destinationCode: 'BDA' },
  'turks and caicos': { destination: 'Providenciales', country: 'Turks & Caicos', destinationCode: 'PLS' },
  'puerto rico': { destination: 'San Juan', country: 'Puerto Rico', destinationCode: 'SJU' },
  'san juan': { destination: 'San Juan', country: 'Puerto Rico', destinationCode: 'SJU' },
  'st lucia': { destination: 'St. Lucia', country: 'St. Lucia', destinationCode: 'UVF' },
  barbados: { destination: 'Barbados', country: 'Barbados', destinationCode: 'BGI' },
  curacao: { destination: 'Curaçao', country: 'Curaçao', destinationCode: 'CUR' },
  ...
  'sao paulo': { destination: 'São Paulo', country: 'Brazil', destinationCode: 'SAO' },
  'buenos aires': { destination: 'Buenos Aires', country: 'Argentina', destinationCode: 'BUE' },
  argentina: { destination: 'Buenos Aires', country: 'Argentina', destinationCode: 'BUE' },
  patagonia: { destination: 'Bariloche', country: 'Argentina', destinationCode: 'BRC' },
  lima: { destination: 'Lima', country: 'Peru', destinationCode: 'LIM' },
  peru: { destination: 'Lima', country: 'Peru', destinationCode: 'LIM' },
  cusco: { destination: 'Cusco', country: 'Peru', destinationCode: 'CUZ' },
  'machu picchu': { destination: 'Cusco', country: 'Peru', destinationCode: 'CUZ' },
  bogota: { destination: 'Bogotá', country: 'Colombia', destinationCode: 'BOG' },
  colombia: { destination: 'Bogotá', country: 'Colombia', destinationCode: 'BOG' },
  cartagena: { destination: 'Cartagena', country: 'Colombia', destinationCode: 'CTG' },
  medellin: { destination: 'Medellín', country: 'Colombia', destinationCode: 'MDE' },
  santiago: { destination: 'Santiago', country: 'Chile', destinationCode: 'SCL' },
  chile: { destination: 'Santiago', country: 'Chile', destinationCode: 'SCL' },
  quito: { destination: 'Quito', country: 'Ecuador', destinationCode: 'UIO' },
  ecuador: { destination: 'Quito', country: 'Ecuador', destinationCode: 'UIO' },
  galapagos: { destination: 'Galápagos', country: 'Ecuador', destinationCode: 'GPS' },
  montevideo: { destination: 'Montevideo', country: 'Uruguay', destinationCode: 'MVD' },
  uruguay: { destination: 'Montevideo', country: 'Uruguay', destinationCode: 'MVD' },
};
...
export async function parseIntent(prompt) {
  ...
  if (dates) {
    ...
    if (dates.nights) intent.nights = dates.nights; // explicit range wins over guesses
  }
  return intent;
}
...
function deriveDates(prompt, nights) {
  ...
  // 0) Explicit range: "June 19 to June 26", "June 19-26", "Dec 28 to Jan 2".
  // The end date is the user's checkout day — honor it exactly.
  const monthAlt = Object.keys(MONTHS).join('|');
  const range = lower.match(new RegExp(
    `\\b(${monthAlt})\\s+(\\d{1,2})(?:st|nd|rd|th)?\\s*(?:to|through|until|till|thru|[-–—])\\s*(?:(${monthAlt})\\s+)?(\\d{1,2})(?:st|nd|rd|th)?\\b`
  ));
  if (range) {
    ...
    if (new Date(year, m1, d1) < now) year += 1;
    const start = new Date(year, m1, d1);
    let end = new Date(m2 < m1 ? year + 1 : year, m2, d2); // "Dec 28 to Jan 2" rolls year
    if (end <= start) end = addDays(start, Math.max(1, nights || 6)); // nonsense range → fall back
    return {
      departureDate: isoDate(start),
      returnDate: isoDate(end),
      checkin: isoDate(start),
      checkout: isoDate(end),
      label: prettyRange(start, end),
